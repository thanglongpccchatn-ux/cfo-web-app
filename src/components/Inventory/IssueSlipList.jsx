    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-rose-50 dark:bg-rose-900/15 border border-rose-100 dark:border-rose-900/30">
          ...
          <span className="text-[13px] font-bold text-slate-600 dark:text-slate-300">{filtered.length} phiếu · Tổng xuất</span>
          <span className="font-mono font-black text-rose-600 text-[15px]">{money(grandTotal)}đ</span>
        </div>
                  ...
                  <tbody>
                    {s.lines.map((l, i) => (
                      <tr key={i} className="border-b border-slate-50 dark:border-slate-700/30">
                        ...
                        <td className="py-1.5 text-right tabular-nums text-slate-500">{money(l.unit_price)}</td>
                        <td className="py-1.5 text-right tabular-nums font-semibold">{money(l.quantity * l.unit_price)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
