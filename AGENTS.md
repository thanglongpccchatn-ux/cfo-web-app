...
- **Bộ component chuẩn `src/components/ui/`** (Card, Button, Badge, PageHeader,
  EmptyState, SkeletonTable): màn hình MỚI bắt buộc dùng, màn cũ thay dần —
  KHÔNG tự chế card/nút/badge mới bằng div thô nữa. Import: `from './ui'`.
- Bóng đổ: chỉ dùng 3 tầng ngữ nghĩa `shadow-card` (nghỉ) / `shadow-raised`
  (nổi, hover) / `shadow-overlay` (modal) — không dùng shadow-sm/md/lg/xl tùy hứng.
- Mobile: pattern chuẩn = bảng desktop `hidden md:block` + card list `md:hidden`,
  ...
  phân trang mobile rút gọn Trước/Sau.
- **Dark mode** (bật/tắt bằng nút ở Header, hook `src/hooks/useDarkMode.js`):
  phần tử mới vẫn viết class `dark:` tử tế. Màn chưa có `dark:` được lớp CSS
  phủ tự động trong index.css (khối "DARK MODE — LỚP PHỦ TỰ ĐỘNG") lo —
  lớp phủ dùng `:where()` nên `dark:` viết tay luôn thắng. RIÊNG
  DashboardOverview + DashboardDetailModal gắn class `light-scope` ở root:
  luôn giữ nền sáng, đừng thêm `dark:` và đừng gỡ `light-scope` ở 2 màn đó.
